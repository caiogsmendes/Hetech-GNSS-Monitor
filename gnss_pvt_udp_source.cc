...
void Gnss_Pvt_Udp_Source::print_monitor(gnss_sdr::MonitorPvt& monitor) const
{
    double pos_x = monitor.pos_x();  // Position X component in ECEF, expressed in m
    double pos_y = monitor.pos_y();  // Position Y component in ECEF, expressed in m
    double pos_z = monitor.pos_z();  // Position Z component in ECEF, expressed in m
    double vel_x = monitor.vel_x();  // Velocity X component in ECEF, in m/s
    double vel_y = monitor.vel_y();  // Velocity Y component in ECEF, in m/s
    double vel_z = monitor.vel_z(); // Velocity Z component in ECEF, in m/s
    ...
    double lat = monitor.latitude();  // Latitude, in deg. Positive: North
    double longt = monitor.longitude(); // Longitude, in deg. Positive: East
    double alt = monitor.height();    // Height, in m
